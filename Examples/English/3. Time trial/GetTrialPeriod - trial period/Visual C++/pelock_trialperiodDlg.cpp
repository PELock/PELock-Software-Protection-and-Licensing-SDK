...
BOOL CPelock_trialperiodDlg::OnInitDialog()
{
	...
	{
		m_PeriodStart.Format("Period start %u.%u.%u", stPeriodStart.wDay, stPeriodStart.wMonth, stPeriodStart.wYear);
		m_PeriodEnd.Format("Period end %u.%u.%u", stPeriodEnd.wDay, stPeriodEnd.wMonth, stPeriodEnd.wYear);
	}
	else
	{
		// full version code
		m_PeriodStart = "No trial limits.";
		m_PeriodEnd = "";
	}
